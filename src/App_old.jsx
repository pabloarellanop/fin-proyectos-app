...
export default function App(){
  ...
  const kpis = useMemo(()=>{
    const totalIncome = dashboardTx.filter(t=>t.kind==="Ingreso").reduce((a,b)=>a+b.amount,0);
    const totalExpense = dashboardTx.filter(t=>t.kind==="Egreso").reduce((a,b)=>a+b.amount,0);
  ...
  }, [dashboardTx, state.ccPurchases]);

  const expenseByCategory = useMemo(()=>{
    const map = new Map();
    dashboardTx.filter(t=>t.kind==="Egreso").forEach(t=>{
  ...
  const incomeByCategory = useMemo(()=>{
    const map = new Map();
    dashboardTx.filter(t=>t.kind==="Ingreso").forEach(t=>{
      const key = t.category || "Sin categoría";
      map.set(key, (map.get(key)||0) + t.amount);
    });
    return [...map.entries()].map(([name, value])=>({ name, value })).sort((a,b)=>b.value-a.value);
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      ...
                      <Legend
                       ...
                       wrapperStyle={{ fontSize: 12, lineHeight: "14px" }}
                     />
                    </PieChart>
                  </ResponsiveContainer>
                </div>
  ...
  {incomeByCategory.map((row, i) => {
    ...
    return <Cell key={i} fill={color || fallback} />;
          ...
          <Section title="Últimos movimientos (caja)">
            <table>
              <thead>
              ...
              <tbody>
                ...
                {dashboardTx.length===0 && <tr><td colSpan={6} className="small">Sin movimientos aún.</td></tr>}
              </tbody>
            </table>
          </Section>
        </div>
